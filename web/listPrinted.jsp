<%-- 
    Document   : listPrinted
    Created on : Mar 20, 2021, 12:05:05 PM
    Author     : Dell Inc
--%>
...
<html>
    <head>
        ...
        <script src="js/pagingPrinted.js" type="text/javascript"></script>
    ...
    <body>
        <header>
            ...
            <nav>
                ...
                <a href="insertPrinted?id=${requestScope.id}">Ghi sổ</a>
                </c:if>
        ...
        <div>
            <table>
                ...
                <c:forEach items="${requestScope.pgs}" var="pg">
                    <tr>
                        <td>${pg.date}</td>
                        <td>${pg.pattern.name}</td>
                        <td>${pg.quantity}</td>
                        <td>${pg.pattern.price}</td>
                        <c:if test="${requestScope.group == 1}">
                        <td><a href="updatePrinted?id=${requestScope.id}&date=${pg.date}">Sửa</a></td>
                        </c:if>
                    </tr>
                </c:forEach>
            </table>
    ...
    <script>
        ...
        function mess(id){
            ...
            if(option===true){
                window.location.href = 'deletePrinted?id='+id;
            }
        }
    </script>
</html>
